...
**目前版本：** [0.40.0](./version.md) · 變更見 [changelog.md](./changelog.md) · 使用前請讀 [DISCLAIMER.md](./DISCLAIMER.md)
...
setup 時可選用哪個 agent；也可用環境變數 `ENGRAM_AGENT=claude`｜`cursor`｜`codex`｜`pi`（預設 **claude**；見 [server/README.md](./server/README.md)）。僅跑 UI／讀既有記憶時可不叫 agent；**入夢與 Ask 沒有可用的 CLI 會失敗**。
