...
本檔規範如何寫 `roadmap/`，讓 **另一個沒有對話紀錄的 agent** 也能正確開工。  
...
| 排程 | `roadmap/X.Y.Z/INDEX.md`（可先 rough） | 有版本號與產品句 |
...
- 若改了 API／操作邊界／詞彙：同步 `api-docs/`、`AGENTS.md`、`domain-language.md`（列進最後一軌較不易漏）
