...
> **目的：** 用最小可跑系統驗證 MVP 句——  
> **≤3 個 node + L0 + L1 + `dream run`（what + day + candidates + L1.5）是否比全量 rewrite 更穩？**  
...
| 題 | 定案 |
|----|------|
| Extract 部分 patch schema 不過 | **整段失敗**（不清 L1、不寫 L1.5）；不採「丟壞留好」 |
| `dream/run` 中途 crash | 若本 run 已有 L1.5、尚未清 L1 → **同 `dream_run_id` 只 resume apply**（不重跑 extract）；L1 已清才允許新 extract／新 id |
...
- extract **整段失敗**（非 JSON / 任一 patch schema 不過 / process exit ≠ 0）→ throw；API 回 `dream_incomplete`；**不清 L1、不寫 L1.5**
...
| `episodic` ≥ 0.6 | **P1–P2：不 apply 正文**（可略過或只留 L1.5）；`chronology/recent.md` → Phase 4 |
...
- Fixture runner：`fixtures/*.jsonl` → 等同 extract 已寫好的 L1.5，只跑 apply
...
- `POST /dream/run` = lock → extract → append L1.5 → apply → clear L1
...
1. ingest 數條真實文字 → `dream/run` → L1.5 有 patches、L2／candidates 有寫入、L1 空  
2. extract 故意弄壞（壞 prompt／mock fail）→ L1 仍在、可重跑  
3. 同 `dream_run_id` 不重複 append L1.5
