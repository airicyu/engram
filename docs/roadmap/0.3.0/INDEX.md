...
| 5 | 誰寫釐清 | **只 distill skill** 在沉澱時若事件缺關鍵事實、且問了有用，才 **新建** asking（一事一問，避免連珠炮）。Ingest／`POST /events` **不**生題。Server **不**生成問題。 |
| 6 | 人怎麼答 | 機械：`POST /clarify/asking/{id}/submit` body `{ "answer": "…" }` → 把答案寫進該檔、**移到** `pending/`。`DELETE /clarify/asking/{id}` 丟到 history 並標 dismissed，不再問。`POST /clarify/aside` body `{ "raw": "…" }` 直接在 `pending/` 新建一筆順帶補充（**不是** pool 事件）。缺檔 submit／delete → 200＋`present: false` 或等價空成功（冪等），不要 404 當「沒這題」。非法 id → 400。 |
...
| 10 | 事件帶圖 | `POST /events` 仍要非空 `raw`。`raw` 可含精確 `![[_attachments/uploads/{day}/{file}]]`（禁 `\|alias`）。可選 `attachments: [{ path, relationship }]`；若有：集合須與 embed 路徑相同、relationship trim 非空、檔須已在正式目錄。Server **不**組 Engram 式 appendix；關係只存在 `attachments[]`（寫進該行 JSON）。無附件則行為與 0.2 相同。 |
...
| 15 | 空讀 | 列表／搜尋無命中／無釐清題 → **200** 空陣列。404 只給未知路由或附件 path 消毒後檔不存在。 |
...
- [ ] Distill 可在 asking 寫出一題；submit 後檔在 `clarify/pending/`；再 distill 後該檔在 `history/` 且相關 node／day 有吸收
- [ ] aside 不進 `pool/pending.jsonl`
- [ ] 上傳 png 201 且磁碟在 `_attachments/uploads/{day}/`；`POST /events` embed＋`attachments[]` 對稱；缺檔或 alias embed → 400
...
| B 釐清 | 目錄＋機械 POST／DELETE；distill skill 段；郵箱 UI | server 出題；approve |
