...
- **已出貨：** `0.31.0` — Hash 深鏈＋MdBlock wikilink 可點＋chain 寫入時 node 互指（**不**回填歷史）；見 `docs/roadmap/0.31.0/`（**shipped**；**無** store migrate）
- **上一版：** `0.30.0` — 釐清（Clarify）：補問＋順帶補充 → 入夢蒸餾進 nodes — 見 `docs/roadmap/0.30.0/`（**shipped**；**無** store migrate）
