...
| Area | Path |
|------|------|
| L1 pool | `memories/short-term-memory/pool.jsonl`, `memories/short-term-memory/summary.md` |
...
Do **not** read `memories/future-sight/`. Do **not** scan all of `memories/activities/events.jsonl` unless necessary — prefer L1, L2, chain.

## Rules (STRICT)

1. **Read only** from the memory store under `{{ENGRAM_STORE_DIR}}` (L1, L2, chain, activities). Do **not** edit any of those paths.
