...
export interface DreamRunState {
  ...
  retry_reason?: string;
  /** Commit succeeded but clearing short-term scope failed — retry clear on next approve. */
...
export async function getShortTermClearPendingRun(): Promise<DreamRunState | null> {
  const runs = await listDreamRuns();
