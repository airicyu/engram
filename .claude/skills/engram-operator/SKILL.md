...
| `POST /ingest` to capture | Append to `events.jsonl` by hand |
| `POST /dream/run` → pending → `approve`／`discard` | Hand-edit L1／L2／draft during review |
| `GET /activate` to read context | Assemble context by reading markdown files |
...
| Term | Meaning |
|------|---------|
| **Ingest** | `POST /ingest` — L0 + L1 pool entry |
| **Extract** | `POST /dream/run` — patches + draft + report; **does not** write L2 |
| **Approve** | `POST /dream/approve` — `commitDraft` → L2, clear scope S |
| **Discard** | `POST /dream/discard` — drop pending; L1／L2 unchanged |
| **Activate** | `GET /activate` — L1, day chain, matched L2 |
...
|----------|---------------|---------|
| `POST /ingest` | `raw` (not `content`／`text`) | `event_id` |
...
| `GET /activate` | `q` optional | `l1`, `chain`, `nodes` |
...
./.claude/skills/engram-operator/scripts/engram-api.sh pending
./.claude/skills/engram-operator/scripts/engram-api.sh approve
./.claude/skills/engram-operator/scripts/engram-api.sh discard
