...
description: 把 Engram Lite 記憶庫重設成初始空狀態（清空 pending／archived／chain／nodes／jobs，保留 workspace.yaml 與目錄骨架）。在使用者說 reset、清空記憶、恢復 initial data 時使用。破壞性：先確認再跑 script。
---
...
這是**破壞性**操作。先向使用者確認要清空目前所有事件、日記、節點、job。沒有明確同意就停止。
